...
| `/codex_image_model refresh` | 立即重新获取官方目录，明确提示刷新成功及模型数量，或刷新失败及备用来源；不改变手动选模设置 |
...
- `/codex_image_model refresh` 在线获取成功后，明确显示“图片生成模型刷新成功”、本次获取的模型数量和自动模式当前选择。
- 在线刷新失败时明确提示使用的是上次成功目录还是内置备用目录，不会把缓存回退显示为刷新成功。
